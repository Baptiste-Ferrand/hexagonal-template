...
from src.entrypoints.api.schemas.profile import EmailUpdate, PasswordUpdate, ProfileCreate, ProfileRead, ProfileWithToken, TokenResponse, ProfileLogin
from src.domain.exceptions import DuplicateProfileError, InvalidConfirmPasswordError, NotFoundError, AuthenticationError
from src.entrypoints.api.deps.auth import UserPayload, get_current_user, require_owner_profile
...
@router.get("/users", response_model= list[ProfileRead], status_code=status.HTTP_200_OK, dependencies=[Depends(get_current_user)])
...
@router.delete("/{profile_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(require_owner_profile)])
...
@router.patch(
    ...
    dependencies=[Depends(require_owner_profile)]
)
